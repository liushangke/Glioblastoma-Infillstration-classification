...
# Extract indices for the 'None' samples
nan_indices = [i for i, label in enumerate(labels) if label == 'nan']
print(len(nan_indices))
# Exclude 'nan' samples from the original data for the stratified split
data_no_nan = np.delete(data, nan_indices, axis=0)
labels_no_nan = np.delete(labels, nan_indices, axis=0)

kf = StratifiedKFold(n_splits=4, shuffle=True, random_state=42)

for fold, (train_idx, val_idx) in enumerate(kf.split(data_no_nan, labels_no_nan)):
    train_losses = []
    val_losses = []
    train_accuracies = []
    val_accuracies = []
    ...
    # Create a mapping of indices from data_no_nan to original data
    mapping = [i for i in range(len(data)) if i not in nan_indices]

    # Map the train and validation indices from data_no_nan to original data
    train_idx = [mapping[i] for i in train_idx]
    val_idx = [mapping[i] for i in val_idx]

    # Adjust indices to include 'nan' samples
    if fold % 2 == 0:  # For even folds
        train_idx = np.concatenate([train_idx, [nan_indices[0]]])
        val_idx = np.concatenate([val_idx, [nan_indices[1]]])
    else:  # For odd folds
        train_idx = np.concatenate([train_idx, [nan_indices[1]]])
        val_idx = np.concatenate([val_idx, [nan_indices[0]]])
    ...
    # Reset the train and validation data/labels using the new indices
    ...
    # Count occurrences of each label in the training and validation sets
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, targets, _ in train_loader:
            ...
            correct_train += (predicted == torch.argmax(targets, dim=1)).sum().item()
        ...
        for inputs, targets, patient_idxs in val_loader:
            ...
            correct_val += (predicted == torch.argmax(targets, dim=1)).sum().item()
